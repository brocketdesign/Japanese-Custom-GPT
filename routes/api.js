   ...
   // This route handles streaming chat completions from OpenAI for a given session ID.
    ...
    fastify.post('/api/prompts/create', async (request, reply) => {
        ...
        fastify.post('/api/models/averageTime', async (req, reply) => {
            ...
            // --- New: Top Free Chats Route using premium field, cache 50, return 3 random each time ---
            fastify.get('/api/top-free-chats', async (request, reply) => {
                try {
                    const cacheLimit = 50; // How many to fetch/cache
                    const returnLimit = 10; // How many to return
                    const language = request.lang;
                    const db = fastify.mongo.db;
                    const chatsCollection = db.collection('chats');
                    const cacheCollection = db.collection('topFreeChatsCache'); // Cache collection

                    let chatsToSelectFrom = [];
                    ...
                    if (
                        cacheDoc &&
                        Array.isArray(cacheDoc.chats) &&
                        cacheDoc.updatedAt &&
                        (now - cacheDoc.updatedAt < cacheExpiry) &&
                        cacheDoc.chats.length >= returnLimit // Ensure cache has enough to select from
                    ) {
                        // Cache is valid and sufficient
                        chatsToSelectFrom = cacheDoc.chats;
                        servedFromCache = true;
                        console.log(`[API /top-free-chats] Served lang ${language} from cache.`);
                    } else {
                        // 2. Cache invalid or insufficient, fetch from DB
                        console.log(`[API /top-free-chats] Cache miss/invalid for lang ${language}. Fetching from DB.`);
                        const pipeline = [
                            // Match free, SFW, language, and basic requirements
                            {
                                $match: {
                                    $and: [
                                        // Accept premium: false, undefined, or not present
                                        { $or: [ { premium: { $exists: false } }, { premium: false }, { premium: null }, { premium: undefined } ] },
                                        // Accept nsfw: false, undefined, or not present
                                        { $or: [ { nsfw: { $exists: false } }, { nsfw: false }, { nsfw: null }, { nsfw: undefined } ] },
                                        { chatImageUrl: { $exists: true, $ne: '' } },
                                        { name: { $exists: true, $ne: '' } },
                                        { language }
                                    ]
                            ...
                            // Use unwind + preserve to handle missing users gracefully
                            ...
                            // Project final fields
                            {
                                $project: {
                                    ...
                                    imageCount: 1,
                                    userId: 1,
                                    nickname: '$userInfo.nickname', // Get nickname directly
                                    profileUrl: '$userInfo.profileUrl', // Get profileUrl directly
                                    language: 1,
                                    nsfw: 1,
                                    premium: 1,
                                    moderation: 1
                                    // Don't include gallery and userInfo (they'll be excluded implicitly)
                                }
                            }
                        ];

                        chatsToSelectFrom = await chatsCollection.aggregate(pipeline).toArray();

                        // 3. Update cache if we fetched data
                        if (chatsToSelectFrom.length > 0) {
                             console.log(`[API /top-free-chats] Updating cache for lang ${language} with ${chatsToSelectFrom.length} chats.`);
                            await cacheCollection.updateOne(
                                { language },
                                { $set: { chats: chatsToSelectFrom, updatedAt: Date.now() } },
                                { upsert: true }
                            );
                        } else {
                             console.log(`[API /top-free-chats] No chats found for lang ${language} to cache.`);
                        }
                    }

                    // 4. Filter out chats with finalNsfwResult true
                    chatsToSelectFrom = chatsToSelectFrom.filter(chat => {
                        // Treat undefined/null as false for nsfw and premium
                        ...
                        // Also ensure premium is not true/'true'
                        const premium = chat?.premium === true || chat?.premium === 'true';
                        return !finalNsfwResult && !premium;
                    });

                    // 5. Select random chats
                    let resultChats = [];
                    if (chatsToSelectFrom.length <= returnLimit) {
                        resultChats = chatsToSelectFrom; // Return all if less than or equal to limit
                    } else {
                        // Shuffle and pick 'returnLimit'
                        const shuffled = chatsToSelectFrom
                            .map(value => ({ value, sort: Math.random() }))
                            .sort((a, b) => a.sort - b.sort)
                            .map(({ value }) => value);
                        resultChats = shuffled.slice(0, returnLimit);
                    }

                    // 6. Send response
                    reply.send({ chats: resultChats, cached: servedFromCache });

                } catch (err) {
                    console.error('[API /top-free-chats] Error:', err);
                    reply.code(500).send('Internal Server Error');
                }
            });
