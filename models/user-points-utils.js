...
async function awardVideoMilestoneReward(db, userId, fastify = null) {
  const usersCollection = db.collection('users');
  const videosCollection = db.collection('videos');  // Change this
  const milestonesCollection = db.collection('user_milestones');
...
async function awardCharacterVideoMilestoneReward(db, userId, chatId, fastify = null) {
  const usersCollection = db.collection('users');
  const videosCollection = db.collection('videos');  // Add this
  const milestonesCollection = db.collection('user_milestones');
  ...
  // Count videos generated for this specific character/chat from videos collection
  const totalVideos = await videosCollection.countDocuments({
    userId: new ObjectId(userId),
    chatId: new ObjectId(chatId)
  });
