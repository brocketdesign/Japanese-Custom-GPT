   ...
   // This route handles streaming chat completions from OpenAI for a given session ID.
    ...
    fastify.get('/api/top-free-chats', async (request, reply) => {
        try {
            ...
            const returnLimit = 3; // How many to return
            ...
            if (
                ...
                const pipeline = [
                    // Match free, SFW, language, and basic requirements
                    {
                        $match: {
                            $and: [
                                // Check for premium: false or non-existent
                                { $or: [ { premium: { $exists: false } }, { premium: false } ] },
                                // Check for nsfw: false or non-existent
                                { $or: [ { nsfw: { $exists: false } }, { nsfw: false } ] },
                                { chatImageUrl: { $exists: true, $ne: '' } },
                                { name: { $exists: true, $ne: '' } },
                                { language }
                            ]
