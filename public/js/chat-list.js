...
function updateHorizontalChatMenu(currentChatId) {
    if ($('#horizontal-chat-list').length === 0) return;
    
    // Remove active indicators from all thumbs
    $('#horizontal-chat-list .chat-thumb-container').each(function() {
        $(this).find('.position-absolute').remove();
    });
    
    // Add active indicator to current chat
    if (currentChatId) {
        const currentThumb = $(`#horizontal-chat-list .chat-thumb-container[data-id="${currentChatId}"]`);
