...
async function img2videoRoutes(fastify) {
    ...
    fastify.post('/api/img2video/generate', async (request, reply) => {
        ...
        try {
            const { imageId, prompt, chatId, userChatId } = request.body;
            ...
            if (!imageId || !chatId) {
                console.warn('[img2video] Missing required parameters', { imageId, chatId });
                return reply.status(400).send({ error: 'Missing required parameters' });
            }

            // Fetch image data from database
            const db = fastify.mongo.db;
            console.log(`[img2video] Fetching gallery for chatId: ${chatId}, userId: ${userId}`);

            // Check if a video is already being generated by the user
            ...
            const gallery = await db.collection('gallery').findOne({ chatId: new ObjectId(chatId), userId: new ObjectId(userId) });
            ...
            console.log(`[img2video] Generated placeholderId: ${placeholderId}`);
            ...
            try {
                ...
                console.log(`[img2video] System prompt:`, systemPrompt);
                const instructionPrompt = await generateCompletion(systemPrompt, 700, 'mistral');
